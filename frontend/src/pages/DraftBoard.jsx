...
import { TrendingUp, TrendingDown, Flame, Star, GraduationCap, Sparkles, ExternalLink, ChevronDown } from "lucide-react";

const SECTIONS = [
  { key: "elites",    title: "Elite Tier",           icon: Star,        accent: "bg-emerald-500",    desc: "Established top-shelf fantasy producers." },
  { key: "breakouts", title: "Breakouts",             icon: Flame,       accent: "bg-emerald-500/30", desc: "Players whose recent trajectory points up — stats already back it up." },
  { key: "sleepers",  title: "Sleepers",              icon: TrendingUp,  accent: "bg-blue-500/40",    desc: "Speculative upside picks — limited experience but promising path." },
...
const ROOKIE_FILTERS = [
  { v: "ALL",          l: "All Rookies" },
  { v: "elite_landing",l: "Elite Landing Spots" },
  { v: "sleeper",      l: "Rookie Sleepers" },
    ...
    <div className="min-h-screen bg-[#0a0e16]">
      ...
      <div className="border-b border-slate-800 bg-slate-950/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-[10px] font-bold tracking-[0.25em] uppercase text-emerald-400 mb-2">◆ The Lab · Draft Board</div>
          <h1 className="font-display text-4xl sm:text-5xl font-black tracking-tight text-white">Draft Board</h1>
          <p className="text-slate-400 mt-2 max-w-2xl">
            Players grouped by fantasy outlook — auto-tagged from stat trajectories. Elites and breakouts ranked highest,
            sleepers are speculative, bust risks flagged. Rookie class below.
          </p>
        </div>
      </div>
                ...
                {list.map((p) => (
                  <li key={p.id} className="px-5 py-3 border-b border-slate-800 last:border-b-0 flex items-center justify-between hover:bg-slate-900">
                    <div className="flex items-center gap-3">
                      ...
                      <Link to={`/stats`} className="font-semibold text-white hover:text-emerald-400">{p.name}</Link>
                      <span className="text-xs font-mono-tab text-slate-500">{p.team}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <TagBadge tag={p.tag} />
                      <span className="font-mono-tab text-sm text-emerald-300 font-bold">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="text-[10px] font-bold tracking-[0.25em] uppercase text-emerald-400">
...
function RookieCard({ rookie, expanded, onToggle, outlook }) {
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <div className="border-t border-emerald-500/20 bg-slate-950/40 p-4">
